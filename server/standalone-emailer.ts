...
if (missingEnvVars.length > 0) {
  console.error(`Variáveis de ambiente necessárias: ${missingEnvVars.join(', ')}`);
  process.exit(1);
}
...
// Template de email em HTML com placeholders
const emailTemplate = `
<!DOCTYPE html>
<html lang="pt-BR">
<head>
...
app.post('/api/invite', async (req, res) => {
  try {
    ...
    console.log(`Processando envio de convite para ${email} (${nome})`);
...
app.listen(PORT, () => {
  ...
  console.log(`Configuração SMTP: ${process.env.SMTP_HOST}:${process.env.SMTP_PORT}`);
  console.log(`Usuário SMTP: ${process.env.SMTP_USER}`);
});